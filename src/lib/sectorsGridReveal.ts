 ...
 * Unaffected by the REPLAY note below either way.
 *
 * REPLAY — the shared timeline's own `ScrollTrigger` uses
 * `toggleActions: "play none none none"`. PR #26 briefly shipped
 * `"play reverse play reverse"` (replay on every re-entry, both
 * directions — scrolling back up would draw the line back in, slide
 * the tabs back out, and fade the panel back down) — REFINED here so
 * scrolling back UP past an already-played SectorsGrid can never undo
 * any of that, only a fresh downward `onEnter` ever plays the sequence.
 * `onEnter` stays `play`; every other position is `none`. See
 * `scrollReveal.ts`'s own note for the full traced sequence (down
 * plays the whole three-beat timeline once, back up leaves the line
 * drawn/tabs settled/panel visible exactly as they landed, and a later
 * `onEnter` on an already-complete timeline is a no-op, not a visible
 * replay of the sequence). The mobile auto-nudge (`maybeNudgeTablist`,
 * wired to this same trigger's `onEnter`) is unaffected either way —
 * `hasNudged` was always its own true once-per-page-load flag,
 * independent of whatever `onEnter` does to the reveal timeline itself.
 *
